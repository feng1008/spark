...
aws emr put --cluster-id $cluster_id --key-pair-file ./key/id_rsa --src ./spark/pro_tm_pairs.py --dest /home/hadoop/pro_tm_pairs.py
...
PRO_PATH=s3://datamining.ym/log_analysis/province/${THIS_DAY}
TIME_PATH=s3://datamining.ym/log_analysis/time/${THIS_DAY}
PAIRS_PATH=s3://datamining.ym/log_analysis/pairs/${THIS_DAY}

step_result=`aws emr add-steps --cluster-id $cluster_id --steps Type=CUSTOM_JAR,Name=GetLast5Package,Jar=s3://cn-north-1.elasticmapreduce/libs/script-runner/script-runner.jar,ActionOnFailure=TERMINATE_CLUSTER,Args=[/home/hadoop/spark/bin/spark-submit,--name,"get letv_pro_tm_pairs $DATE",--master,$SPARK_MASTER,--executor-memory,3G,--total-executor-cores,20,/home/hadoop/pro_tm_pairs.py,$LOG_PATH,$PRO_PATH,$TIME_PATH,$PAIRS_PATH]`
