...
CLUSTER_NAME="awscn-up-emr-buysCount"
...
aws emr put --cluster-id $cluster_id --key-pair-file ./key/id_rsa --src ./spark/buys_count.py --dest /home/hadoop/buys_count.py
...
INPUT_PATH=s3://datamining.ym/buysCount/data.csv
OUTPUT_PATH=s3://datamining.ym/buysCount/${DATE}

step_result=`aws emr add-steps --cluster-id $cluster_id --steps Type=CUSTOM_JAR,Name=buysCount,Jar=s3://cn-north-1.elasticmapreduce/libs/script-runner/script-runner.jar,ActionOnFailure=TERMINATE_CLUSTER,Args=[/home/hadoop/spark/bin/spark-submit,--name,"Count buys $DATE",--master,$SPARK_MASTER,--executor-memory,3G,--total-executor-cores,10,/home/hadoop/buys_count.py,$INPUT_PATH,$OUTPUT_PATH]`
