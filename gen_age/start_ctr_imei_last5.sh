...
aws emr put --cluster-id $cluster_id --key-pair-file ./key/id_rsa --src ./spark/ctr_imei_last5.py --dest /home/hadoop/ctr_imei_last5.py
...
# while((days_ago>=1))
# do
# LAST_DAY=`date -d"$days_ago day ago" +"%Y-%m-%d"`
...
IMEI_PATH=s3://logarchive.ym/rtb/feed/sv=1/*/ac=0/
LAST5_PATH=s3://datamining.ym/user_profile/last5/
OUTPUT_PATH=s3://datamining.ym/user_profile/commerce/ctr/aos/${DATE}

step_result=`aws emr add-steps --cluster-id $cluster_id --steps Type=CUSTOM_JAR,Name=interest,Jar=s3://cn-north-1.elasticmapreduce/libs/script-runner/script-runner.jar,ActionOnFailure=TERMINATE_CLUSTER,Args=[/home/hadoop/spark/bin/spark-submit,--name,"Count interest $DATE",--master,$SPARK_MASTER,--executor-memory,3G,--total-executor-cores,20,/home/hadoop/ctr_imei_last5.py,$IMEI_PATH,$LAST5_PATH,$DAYS_AGO,$OUTPUT_PATH]`
