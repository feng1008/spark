...
aws emr put --cluster-id $cluster_id --key-pair-file ./key/id_rsa --src ./spark/match_imei_last5.py --dest /home/hadoop/match_imei_last5.py
...
OUTPUT_PATH=s3://datamining.ym/user_profile/commerce/sex_last5/aos/${DATE}

step_result=`aws emr add-steps --cluster-id $cluster_id --steps Type=CUSTOM_JAR,Name=interest,Jar=s3://cn-north-1.elasticmapreduce/libs/script-runner/script-runner.jar,ActionOnFailure=TERMINATE_CLUSTER,Args=[/home/hadoop/spark/bin/spark-submit,--name,"Count interest $DATE",--master,$SPARK_MASTER,--executor-memory,3G,--total-executor-cores,20,/home/hadoop/match_imei_last5.py,$USER_last5_PATH,$IMEI_PATH,$OUTPUT_PATH]`
