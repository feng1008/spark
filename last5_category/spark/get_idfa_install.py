...
def main(sc):
    ...
    idfa_output_file=sys.argv[2]

    idfa_data=sc.textFile(input_file).flatMap(parse).filter(list_filter).reduceByKey(add).sortBy(lambda x:int(x[-1]), ascending=False).map(app_map)
