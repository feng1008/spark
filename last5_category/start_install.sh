...
aws emr put --cluster-id $cluster_id --key-pair-file ./key/id_rsa --src ./spark/get_install_dict.py --dest /home/hadoop/get_install_dict.py
...
INPUT_PATH=s3://logarchive.ym/user_packages/1509/aos/
IMEI_PATH=s3://datamining.ym/user_profile/commerce/install_dict/aos/${DATE}

step_result=`aws emr add-steps --cluster-id $cluster_id --steps Type=CUSTOM_JAR,Name=GetLast5Package,Jar=s3://cn-north-1.elasticmapreduce/libs/script-runner/script-runner.jar,ActionOnFailure=TERMINATE_CLUSTER,Args=[/home/hadoop/spark/bin/spark-submit,--name,"get install dict $DATE",--master,$SPARK_MASTER,--executor-memory,3G,--total-executor-cores,20,/home/hadoop/get_install_dict.py,$INPUT_PATH,$IMEI_PATH]`
