...
def main(sc):
    ...
    all_data=cid_data.join(log_data)
    # all_data=cid_data.join(log_data).flatMap(map_bran_scr).cache()
    # cid_data.join(log_data).flatMap(map_bran_scr).saveAsTextFile('result')
    ...
    all_data.saveAsTextFile(all_output)
