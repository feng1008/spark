...
def parse_cid(lines):
    ...
    if imei=='' and ifa=='' or imei!='' and ifa!='':
        return ("", "")
    if imei!='':
        if 'android'+'\t'+imei in bro_log_set.value:
            return ('android'+'\t'+imei, device+'\t'+height+'\t'+width)
        return ("", "")
    if ifa!='':
        if 'ios'+'\t'+ifa in bro_log_set.value:
            device='apple'
...
def map_bran_scr(lines):
    ...
    try:
        ...
        # drt, prv, pns=lines[1].split('\t')
    except:
        return result

    if lines[0] not in bro_cid_dict.value:
        return result
    device, height, width=bro_cid_dict.value[lines[0]].split('\t')
    ...
    for tus in lines[1].split('&'):
        ...
        for app in pns.split('|'):
            if os+'\t'+app in app_top_set.value:
                ...
                if height!='' and width!='':
                    result.append(('hei_wid'+'\t'+height+'_'+width+'\t'+os+'\t'+app,1))
...
def map_log_set(lines):
    return lines[0]

def cid_filter(lines):
    global bro_log_set

    if lines==[] or lines[0] not in bro_log_set.value:
        return False
    return True
...
def main(sc):
    ...
    log_data=sc.textFile(log_file).sample(False, 0.1, 10).map(parse).filter(tuple_filter).reduceByKey(device_reduce)

    log_set=set(log_data.map(map_log_set).collect())
    bro_log_set=sc.broadcast(log_set)
    log_set=None
    ...
    app_set_data.unpersist()

    cid_dict=dict(sc.textFile(cid_file).sample(False, 0.1, 80).map(parse_cid).filter(tuple_filter).collect())
    bro_cid_dict=sc.broadcast(cid_dict)
    cid_dict=None

    all_data=log_data.flatMap(map_bran_scr).filter(list_filter)
    # all_data=sc.textFile(cid_file).map(parse_cid).filter(list_filter).flatMap(map_bran_scr).filter(tuple_filter)

    all_data.filter(filter_prv).reduceByKey(add).map(map_tuple2str).saveAsTextFile(prv_output)
    all_data.filter(filter_drt).reduceByKey(add).map(map_tuple2str).saveAsTextFile(drt_output)
    all_data.filter(filter_device).reduceByKey(add).map(map_tuple2str).saveAsTextFile(device_output)
    all_data.filter(filter_hei_wid).reduceByKey(add).map(map_tuple2str).saveAsTextFile(hei_wid_output)
