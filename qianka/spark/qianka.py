...
def gen_add_feature(user_install, add_feature_file):
    global general_app
    global app_pay_dict

    result=[]

    f1=open(user_install, 'r')
    f2=open(add_feature_file, 'w')
    for lines in f1.readlines():
        fee_c=0
        fee_sum=0.0
        gen_sum=0
        idfa, packages=lines.strip('\r\n').split('\t')
        app_num=len(packages.split('|'))
        if app_num<5:
            # result.append([0.0, 0, 0.0])
            f2.write('\t'.join(['0.0', '0', '0.0'])+'\n')
            continue
        ...
        gen_ratio='%0.4f'%(float(gen_sum)/app_num)
        if fee_c!=0:
            fee_avg='%0.4f'%(log(float(fee_sum)/fee_c))
            fee_c=log(fee_c)
        else:
            fee_avg=str(0.0)
        # result.append([gen_ratio, fee_c, fee_avg]) 
        f2.write('\t'.join([str(gen_ratio), str(fee_c), str(fee_avg)])+'\n')
    f1.close()
    f2.close()

def get_avg_fee(input_file, output_file):
    result=[]
    f1=open(input_file, 'r')
    f2=open(output_file, 'w')
    for lines in f1.readlines():
        idfa, app_str=lines.strip('\r\n').split('\t')
        app_num=0
        app_sum=0.0
        for app_tu in app_str.split('|'):
            app, price=app_tu.split(':')
            app_num+=1
            app_sum+=float(price)
        result.append([app_num, float(app_sum)/app_num])
    f1.close()
...
if __name__=='__main__':
    ...
    gen_add_feature('data_0831/all', 'data_0831/add_feature')
    # get_avg_fee('data_0831/user_fee_app', 'data_0831/user_avg_fee')